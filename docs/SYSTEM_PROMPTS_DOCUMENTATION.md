...
This document contains all the system prompts used in the multi-agent debate system and section evaluator.
...
The section evaluator uses a sophisticated prompt that adapts based on:
1. **Paper type** (empirical, theoretical, policy)
2. **Section type** (introduction, methodology, results, etc.)
3. **Evaluation criteria** (dynamically loaded based on paper type)

#### General Philosophy and Instructions
...
```
This is an EMPIRICAL economics paper. Key characteristics:
...
```
This is a THEORETICAL economics paper. Key characteristics:
...
```
This is a POLICY-FOCUSED economics paper. Key characteristics:
...
```
The INTRODUCTION should follow the "3 moves" structure:
...
### Sophistication Assessment (For Theoretical/Empirical Papers)
...
### Section Evaluator
- **Adapts to paper type**: Different contexts for empirical, theoretical, policy papers
...
- **Cross-domain integration**: Synthesizing multiple perspectives
